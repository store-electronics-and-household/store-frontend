...
const CartItem: FC<Props> = ({
  ...
  return (
    ...
    <div className='cart__item-info'>
      <div className='cart__item-image-wrapper'><img src={imgUrl} alt='Фото товара' /></div>
    ...
    <div className='cart__item-data'>
      <div className='cart__item-price-wrapper'>
        <div className='cart__item-quantity-button'>
            <button className='cart__item-quantity-button-symbol' onClick={() => { decreaseCartQuantity(id); }}>
              {quantity > 1
                ? (<img src={minusIconActive} alt='Уменьшить количество' />)
                : <img src={minusIconDefault} alt='Уменьшить количество' />}
            </button>

            <p className='cart__item-quantity-button-number'>
              {quantity}
            </p>

            <button className='cart__item-quantity-button-symbol' onClick={() => { increaseCartQuantity(id); }}>
              <img
                src={plusIconActive}
    ...
    <div className='cart__cross-button-wrapper'>
      <button className='cart__cross-button' onClick={() => { removeFromCart(id); }}>
        <img
