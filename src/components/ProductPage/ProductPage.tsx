...
const ProductPage: React.FC = () => {
  ...
  return <>
    <Breadcrumb currentPlace={ productSpecifyValue.productName }>
      ...
      <BreadcrumbItem breadcrumbText='Apple iPhone' breadcrumbPath='/catalog'/>
      <BreadcrumbItem breadcrumbText='iPhone 15 Pro' breadcrumbPath='/catalog'/>
    </Breadcrumb>
    <section className='product-page'>
      <div className='product-page__head-container'>
        <h1 className='product-page__header'>{ productSpecifyValue.productName }</h1>
        <span className='product-page__item-number'>Арт. <span>{ productSpecifyValue.article }</span></span>
      </div>
      ...
      <div id='characteristics-anchor' className='product-page__about-product'>
        ...
        isActive
          ? <div className='product-page__about'>
              <p className='product-page__about-description'>Данная модель могла быть активирована до продажи без привязки к аккаунту Apple ID. Это не влияет на потребительские характеристики, внешний вид товара, функциональность или какие-либо иные свойства продукта. Срок гарантии на смартфон составляет 1 год с момента покупки.</p>
              <p className='product-page__about-description'>Смартфон Apple iPhone 15 Pro Max Black Titanium получил дисплей диагональю 6,7 дюйма, выполненный по технологии Super Retina XDR. Разрешение — 2796x1290 пикселей, яркость достигает 2000 кд/м², контрастность составляет 2000000:1 — изображение четкое, детализированное и насыщенное.</p>
              <p className='product-page__about-description'>Основная камера тройная: 48/12/12 Мп. Она способна снимать в разрешении до 4К (3840x2160 пикселей). Оптический зум на увеличение и на уменьшение, цифровая и оптическая стабилизация и вспышка помогут получить отличные результаты в любых условиях. Фокус при съемке портретов можно менять уже после того, как сделан снимок. Для селфи и видеосвязи предназначена фронтальная камера на 12 Мп.</p>
              <p className='product-page__about-description'>Смартфон снабжен универсальным портом USB Type-C, соответствующий кабель для зарядки прилагается. Доступна также беспроводная и быстрая зарядка, поддерживается технология MagSafe. Корпус выполнен из авиационного титана, внутренняя рама — из алюминия (100% переработки), задняя панель отделана стеклом. На боковой панели размещена кнопка Action Button, отвечающая активацию беззвучного режима, включение камеры, фонарика и прочих функций. Заявлена защита по стандарту IP68 — смартфон может без вреда для себя погружаться на глубину до 6 м.</p>
            </div>
          : <ProductCharacteristicsList
            productSpecifyName={ productSpecifyName }
            productSpecifyValue={ productSpecifyValue }
            keysList={ objectKeys(productSpecifyValue).filter((n) => { return n !== 'productName' && n !== 'article' && n !== 'aboutProduct'; }) }
